...
architecture behavioral of oled_ctrl is
    ...
    signal test_counter : std_logic_vector(15 downto 0) := (others => '0');
    signal int_data : unsigned(15 downto 0);
    signal frac_data : unsigned(15 downto 0);

    function bin_to_bcd(bin_in : unsigned(11 downto 0)) return std_logic_vector is
        variable temp : unsigned(11 downto 0) := bin_in;
        variable bcd : unsigned(15 downto 0) := (others => '0'); -- 4 digits
    begin
        for i in 0 to 11 loop
            -- 1. Check if any BCD digit is > 4, if so add 3
            ...
            -- 2. Shift the whole thing left by 1
            bcd := bcd sll 1;
            bcd(0) := temp(11);
            temp := temp sll 1;
        end loop;
...
begin
    ...
    oled_sdin <= init_sdata when current_state = OledInitialize else
        writer_sdata;
    oled_sclk <= init_spi_clk when current_state = OledInitialize else
        writer_spi_clk;
    ...
    init_en <= '1' when current_state = OledInitialize else
        '0';
    writer_en <= '1' when current_state = OledWriter else
        '0';
    -- End enable MUXes
    ...
    begin
        if rising_edge(clk_100) then
            if rst = '1' then
                ...
                test_counter <= bin_to_bcd(frac_data(11 downto 0));
